...
int main(void) {
    ...
    fluid.set_properties(1000.0, 0.001, 0.6, 300, FLUID::LIQUID, 1); // テストモードで設定
    
    vector<double> props = fluid.get_properties(1); // テストモードで呼び出し
    
    std::cout << "Properties of fluid: " 
              << props[0] << ", " << props[1] << ", " 
              << props[2] << ", " << props[3] << ", " 
              << static_cast<int>(props[4]) << std::endl;
